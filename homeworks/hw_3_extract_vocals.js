...
const mainPaths = {
  "tom": {
    ...
    "outDir": path.join(
      "/Users", "tomthecollins", "Shizz", "UMiami", "Teaching", "511-611",
      "spring24", "homeworks", "hw_3", "music_data", "5_hip_hop_vocal_tracks"
    )
  },
  "anotherUser": {
    // ...
...
.forEach(function(file, ithFile){
  ...
  try {
    ...
    const vt = mi.find_vocal_track()
    console.log("vt:", vt)
    if (vt === undefined){
      console.log("No vocal track identified")
      return
    }
    const points = mi.points.filter(function(pt){
      return pt[param.noteIndices.channelIndex] === mi.data.tracks[vt[0]].channel
    })
